<template>
  <main>
    ...
    <div
      class="section relative bg-gray-50 p-12 z-10 m-auto max-w-4xl rounded-2xl overflow-hidden"
    >
      <header class="mb-8">
...
What's up</textarea
        ...
        >
          <h1 class="header-text w-8/12">
            Great! Your message is on it's way to us!
          </h1>
        </div>
      </form>
    </div>
  ...
  methods: {
    async submitForm() {
      ...
      try {
        ...
        const msgs = await this.$axios.$post(
          `${this.$store.state.apiUrl}/visior-messages`,
          data
        )
